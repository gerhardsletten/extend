...
class MeioUploadBehavior extends ModelBehavior {
	...
	function beforeSave(&$model) {
		...
		foreach($this->__fields as $fieldName=>$options) {
			...
			// and !empty($model->data[$model->name][$fieldName]['remove']) and isset($model->data[$model->name][$fieldName]['remove'])
			//array_key_exists('remove', $model->data[$model->name][$fieldName])
			if( !empty($model->data[$model->name][$fieldName]['remove']) and $model->data[$model->name][$fieldName]['remove'] == 1 ){
				if($options['default']){
	...
	function createthumb($name, $filename, $new_w, $new_h)
	{
		...
		if($size[0]<$new_w or $size[1]<$new_h) 
			return false;
